...
class _FakeMessages:
    ...
    def create(self, **kwargs: Any) -> _CreateResponse:
        ...
        return _CreateResponse([_Block("text", self.text)], _Usage(*self.usage))

    def parse(self, **kwargs: Any) -> _ParseResponse:
        self.parse_calls.append(kwargs)
        return _ParseResponse(self.parsed, _Usage(*self.usage))


class FakeAnthropic:
    def __init__(
        self, *, text: str = "", usage: tuple[int, int] = (0, 0), parsed: Any = None
    ) -> None:
        self.messages = _FakeMessages(text=text, usage=usage, parsed=parsed)
